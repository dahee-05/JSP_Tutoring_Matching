...
<script type="text/javascript">
	function deleteChk(){
		var result = confirm("클래스를 삭제하시겠습니까?")
		
		if(result) {
			location.href="tchRegDeletePro.do?classno=${tchclass.classno}&pageNum=${pageNum}";
		} 
	}
</script>
							...
							<div class="table-responsive text-nowrap">
								<table class="table">
									<tbody class="table-border-bottom-0">
										...
										<tr><td>수강대상</td>
											<td>
												<c:if test="${tchclass.grade == 0}">모든 학년</c:if>
												<c:if test="${tchclass.grade != 0}">${tchclass.grade}학년</c:if>
											</td>
										</tr>
										...
										<tr>
											...
											<td>
												<c:forEach var="loc" items="${s_locList }">
													${loc.upper_locname } ${loc.loc_name }<br>
												</c:forEach>
											</td>
										</tr>	
